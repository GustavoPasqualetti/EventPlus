...
const EventosAlunoPage = () => {
  ...
  const loadMyCommentary = async (idUsuario, idEvento) => {
    ...
    try {
      ...
      setidComentarioEvento(myComm.length > 0 ? myComm[0].idComentarioEventoEvento : null);
    } catch (error) {
  ...
  const commentaryRemove = async (idComentarioEvento) => {
    // alert("Remover o comentário " + idComentarioEvento);

    try {
      const promise = await api.delete(
        `${commentaryEventResource}/${idComentarioEvento}`
      );
      if (promise.status === 200) {
    ...
    <>
      ...
      {showModal ? (
        <Modal
          ...
          userId={userData.userId}
          idEvento={idEvento}
          idComentarioEvento={idComentarioEvento}
        />
      ) : null}
    </>
  );
